...
from utils.model_utils import get_all_models, convert2namedtuple, get_all_fine_models


class DeepFeatureMatcher(torch.nn.Module):

    def __init__(self, model: str = 'resnet18', fine_model=None, device="cpu", bidirectional=True,
                 enable_two_stage=True, ratio_th=None, fine_ratio=0.9, up_sample=None, down_sample=16,
                 **kwargs):
        ...
        self.weights = kwargs["fine_weights_path"] if "fine_weights_path" in kwargs.keys() else None
        self.resize = kwargs["resize"] if "resize" in kwargs.keys() else None

        self.up_sample = up_sample

        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") if device is None else device
        self.padding_n = up_sample[0]
        ...
        if fine_model is not None:
            self.fine_model = self.get_fine_model(fine_model)
        else:
    ...
    def get_model(self, model_name: str):
        ...
        assert model_name in models_map.keys(), "Error: model " + model_name + " is not supported!"
        return models_map[model_name](self.ratio_th, self.device, down_sample=self.down_sample)

    def get_fine_model(self, model_name: str):
        model_name = model_name.upper()
        models_map = get_all_fine_models()
        assert model_name in models_map.keys(), "Error: fine model " + model_name + " is not supported!"
        return models_map[model_name](self.device, weights=self.weights)
    ...
    def match(self, img_A, img_B, display_results=0, *args):
        ...
        # get acitvations
        with torch.no_grad():
            activations_A = convert2namedtuple(self.model.forward(inp_A))

            activations_B = convert2namedtuple(self.model.forward(inp_B))
        ...
        if self.enable_two_stage:
            ...
            if self.up_sample[0] != 1:
                points_A = (points_A + 0.5) * self.up_sample[0] - 0.5
                points_B = (points_B + 0.5) * self.up_sample[0] - 0.5
            ...
            if points_A.size(1) >= 4:
                H_init, _ = cv.findHomography(src, dst, method=cv.RANSAC, ransacReprojThreshold=16 * np.sqrt(2) + 1,
            ...
            if display_results:
                ...
                points_C = torch.from_numpy(H_init) @ torch.vstack(
                    (points_B + 0.5, torch.ones((1, points_B.size(1))))).double()
            ...
            if self.fine_model is not None:
                with torch.no_grad():
                    fine_feature_C = convert2namedtuple(self.fine_model.forward(inp_C))
                    fine_feature_A = convert2namedtuple(self.fine_model.forward(inp_A))

            # get activations of the warped image
            with torch.no_grad():
                activations_C = convert2namedtuple(self.model.forward(inp_C))
        else:
            if self.fine_model is not None:
                with torch.no_grad():
                    fine_feature_C = convert2namedtuple(self.fine_model.forward(inp_B))
                    fine_feature_A = convert2namedtuple(self.fine_model.forward(inp_A))
        ...
        if self.up_sample[1] != 1:
            points_A = points_A * self.up_sample[1]
            points_C = points_C * self.up_sample[1]

        if self.fine_model is not None:
            points_A, points_C = refine_points(points_A, points_C, fine_feature_A[0], fine_feature_C[0],
                                               self.fine_ratio, self.bidirectional)
        ...
        # optional
        # in_image = torch.logical_and(points_A[0, :] < (inp_A.shape[3] - padding_A[0] - 16),
        #                              points_A[1, :] < (inp_A.shape[2] - padding_A[1] - 16))
        in_image = torch.logical_and(points_A[0, :] < (inp_A.shape[3] - padding_A[0] - 0),
                                     points_A[1, :] < (inp_A.shape[2] - padding_A[1] - 0))
        in_image = torch.logical_and(in_image,
                                     torch.logical_and(points_B[0, :] < (inp_B.shape[3] - padding_B[0] - 0),
                                                       points_B[1, :] < (inp_B.shape[3] - padding_B[1] - 0)))
        ...
        if points_A.size(1) >= 4:
            H, _ = cv.findHomography(src, dst, method=cv.RANSAC, ransacReprojThreshold=3.0, maxIters=5000,
                                     confidence=0.9999)
            # H, _ = cv.findFundamentalMat(src, dst, method=cv.RANSAC, ransacReprojThreshold=3.0, maxIters=5000,
            #                          confidence=0.9999)
    ...
    def transform(self, img):
        ...
        pad_right = self.padding_n - img.shape[1] % self.padding_n if img.shape[1] % self.padding_n else 0
        pad_bottom = self.padding_n - img.shape[0] % self.padding_n if img.shape[0] % self.padding_n else 0
...
def refine_points(points_A: torch.Tensor, points_B: torch.Tensor, activations_A: torch.Tensor,
                  activations_B: torch.Tensor, ratio_th=0.9, bidirectional=True):
    # normalize and reshape feature maps
    ...
    mask = torch.logical_and(torch.max(ratio_A2B, ratio_B2A) < ratio_th,
                             (ind_B[ind_A] == ind).view(num_input_points, -1))

    if ratio_th == -1.0:
        mask = torch.ones_like(mask)
        mask = torch.logical_and(mask, mask)
...
def mnn_ratio_matcher(descriptors1, descriptors2, ratio=0.8, bidirectional=True):
    ...
    mask = torch.min(ids1 == nn21[nn12], torch.min(ratios12 <= ratio, ratios21[
        nn12] <= ratio))  # discard ratios21 to get the same results with matlab
    # Final matches.
