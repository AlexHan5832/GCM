...
def load_network(model_fn):
    ...
    print("\n>> loading net = " + checkpoint['net'])
    net = Student_Quad_L2Net_ConfCFS()
...
class Student_Quad_L2Net(PatchNet):
    """ Same than L2_Net, but replace the final 8x8 conv by 3 successive 2x2 convs.
    """
...
class Student_Quad_L2Net_ConfCFS(Student_Quad_L2Net):
    """ Same than Quad_L2Net, with 2 confidence maps for repeatability and reliability.
    """

    def __init__(self, **kw):
        Student_Quad_L2Net.__init__(self, **kw)
...
class Coarse_Quad_L2Net_ConfCFS(Quad_L2Net):
    def __init__(self, **kw):
        Quad_L2Net.__init__(self, **kw)
        # reliability classifier
        self.clf = nn.Conv2d(self.out_dim, 2, kernel_size=1)
    ...
    def forward(self, x, **kw):
        for op in self.ops:
            x = op(x)
        feature_dw2 = F.avg_pool2d(x, 2, stride=2)
        feature_dw4 = F.avg_pool2d(feature_dw2, 2, stride=2)
        feature_dw8 = F.avg_pool2d(feature_dw4, 2, stride=2)
        feature_dw16 = F.avg_pool2d(feature_dw8, 2, stride=2)
        outputs = namedtuple("Outputs", ['fmap', 'fmap_dw2', 'fmap_dw4', "fmap_dw8", "fmap_dw16"])
        return outputs(x, feature_dw2, feature_dw4, feature_dw8, feature_dw16)



class Coarse_Student_Quad_L2Net_ConfCFS(Student_Quad_L2Net):
    """ Same than Quad_L2Net, with 2 confidence maps for repeatability and reliability.
    """
    ...
    def forward(self, x, **kw):
        for op in self.ops:
            x = op(x)
        feature_dw2 = F.normalize(F.avg_pool2d(x, 2, stride=2), p=2, dim=1)
        feature_dw4 = F.normalize(F.avg_pool2d(feature_dw2, 2, stride=2), p=2, dim=1)
        feature_dw8 = F.normalize(F.avg_pool2d(feature_dw4, 2, stride=2), p=2, dim=1)
        feature_dw16 = F.normalize(F.avg_pool2d(feature_dw8, 2, stride=2), p=2, dim=1)
        outputs = namedtuple("Outputs", ['fmap', 'fmap_dw2', 'fmap_dw4', "fmap_dw8", "fmap_dw16"])
        return outputs(x, feature_dw2, feature_dw4, feature_dw8, feature_dw16)
...
if __name__ == '__main__':
    ...
    net = load_network(r"D:\hy\Pycharm\IME\Algorithms\DFM\python\models\off-the-shelf\r2d2_WASF_N8_big.pt")

    x = torch.ones((1, 3, 510, 480))
