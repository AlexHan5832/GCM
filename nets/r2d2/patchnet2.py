...
class Quad_L2Net_ConfCFS(Quad_L2Net):
    ...
    def normalize(self, x, ureliability, urepeatability):
        return dict(
                    t_feature_map=x,
                    t_descriptors=F.normalize(x, p=2, dim=1),
                    t_repeatability=self.softmax(urepeatability),
                    t_reliability=self.softmax(ureliability)
                    )
...
if __name__ == '__main__':
    ...
    student = Student_Quad_L2Net_ConfCFS()
    print(student)
    # for i in range(len(student.ops)):
    #     x = student[i](x)
    #     print(x.size())
    x = torch.rand(3, 3, 3, 112, 222)
    print(x.size())
    print(torch.cat(student(x)['repeatability']).shape)
